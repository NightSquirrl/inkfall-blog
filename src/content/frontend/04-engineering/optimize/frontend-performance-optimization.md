...
除了上面提到的优化策略, CSS 还有一个可以影响性能的因素是： _CSS 会阻塞关键渲染路径。_

CSS 是关键资源，它会阻塞关键渲染路径也并不奇怪， _但通常并不是所有的 CSS 资源都那么的『关键』。_
...
为获得最佳性能，_**您可能会考虑将关键 CSS 直接内联到 HTML 文档内**_。这样做不会增加关 键路径中的往返次数，并且如果实现得当，在只有 HTML 是阻塞渲染的资源时，可实现 次往返”关键路径长度。
...
<html>
  <head>
    ...
    <link
      rel="stylesheet"
      href="https://lib.baomitu.com/CSS-Mint/2.0.6/css-mi
    nt.min.css"
    />
  </head>
  <body>
...
<span data-type="text"
  >&lt;script async src=&quot;https://google-analytics.com/analytics.js&quot;&gt;&lt;/script&gt;</span
>
```
